...
from docs_validator.config import ValidatorConfig, load_config_from_toml, merge_config
from docs_validator.core.explorer import FilesExplorer
from docs_validator.core.extractor_factory import get_extractor
from docs_validator.core.models import DocumentationFile, ValidationIssue, LinkStatistics, LinkType, Link
from docs_validator.rules import BrokenLinkValidator, OrphanFileValidator, AnchorLinkValidator, \
...
def load_configuration(args: Namespace) -> ValidatorConfig:
    ...
    config_file = Path(args.config) if args.config else Path.cwd() / '.docs_validator.toml'
    ...
    config = replace(config, **cli_updates)

    if config.path_to_explore is None:
        config=replace(config,path_to_explore=Path('.'))
